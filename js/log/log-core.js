...
window.tratarSwipeLog = function() {
    ...
    const logTabs = ['rotas', 'fretes', 'dashboard', 'frota', 'reps'];
...
window.alternarAbalog = (tab, direction = 'none') => {
    ...
    const secoesSidebar = ['rotas', 'fretes', 'dashboard', 'frota', 'reps'];
    secoesSidebar.forEach(s => {
        const btn = document.getElementById('nav-' + s);
        if (btn) {
            if (s === tab) {
                btn.className = "w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-white/15 text-[#fac043] transition-all font-bold cursor-pointer";
            } else {
                btn.className = "w-full flex items-center gap-3 px-4 py-3 rounded-xl text-slate-300 hover:bg-white/5 hover:text-white transition-all cursor-pointer";
            }
        }
    });
    ...
    const headerInfo = {
        ...
        'fretes': { title: 'Cotação de Frete & Ordem de Carregamento', subtitle: 'Gerenciamento financeiro e emissão de OC', icon: 'fa-calculator' },
        'dashboard': { title: 'Visão Geral & Métricas', subtitle: 'Desempenho e relatórios estratégicos', icon: 'fa-chart-pie' },
        'frota': { title: 'Gestão de Frota e Motoristas', subtitle: 'Controle de motoristas, veículos, reboques e dolly', icon: 'fa-truck-front' },
        'reps': { title: 'Gestão de Representantes & Logística', subtitle: 'Representantes, cobertura de rotas e equipe interna', icon: 'fa-user-tie' }
    };
